<template>
  <div class="cart-wrop">
    <div class="container" v-show="!order">
      ...
      <div class="checkout" v-if="!!productForCart.length">
        <div @click="showCheckSection" type="button" class="btn btn-success">
          Proceed to Checkout
        </div>
      </div>
      <div class="checkout" v-if="!productForCart.length">
      ...
      <Checkout v-bind:class="{active: checkOrder}" @order="getOrder"/>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      scroll() {
        let  y = document.querySelector('.container').getBoundingClientRect().height;
        window.scroll({
          top: y,
          behavior: 'smooth'
        });
